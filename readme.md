...
## Installare le dipendenze frontend (e imparare meglio `npm`)

Da questa esercitazione ho volutamente omesso dei pacchetti frontend per spingerti a usare il comando `npm`.

Infatti mancano Tailwind CSS per lo stile, React Router per gestire le rotte e Axios per effettuare le chiamate HTTP al backend.

Quindi una volta che hai scaricato questa repository, installa le seguenti dipendenze:

- TailwindCSS ([clicca qui per consultare la documentazione](https://tailwindcss.com/docs/installation/using-vite))
- React Router (`npm install react-router`)
- Axios (`npm install axios`)

NOTA: se vuoi scrivere il tuo CSS puoi evitare di installare Tailwind
